...
except OtherPublicHoliday, whose dates are given in the operating profile itself.
...
def bank_holiday_table(division, start, end, holidays=None):
    """
    Map every TransXChange bank-holiday name (except OtherPublicHoliday, which
    carries its own dates) to its dates within [start, end].
    ...
    # OtherPublicHoliday carries its own Date in the profile, so it has no entry
...
def get_bank_holiday_dates(gtfs_info):
    """
    All gov.uk bank holidays (every division of the feed) during the feed
    operative period as GTFS dates (YYYYMMDD), or None if there are none.
    """
    ...
    holidays = read_bank_holidays()
    dates = sorted({day for day in holidays["date"] if start <= day <= end})
    if not dates:
        return None
