"""
Turn the TransXChange model into the per-stop "GTFS info" table from which
stop_times, trips, calendar, calendar_dates, routes and frequencies are derived.
"""
...
def parse_runtime_duration(runtime):
    ...
    if not any(components) or ("T" in value and not any(time_components)):
        raise ValueError("Not an ISO-8601 duration: '%s'" % runtime)
    total = (
        Decimal(parts["days"] or 0) * 86400
        + Decimal(parts["hours"] or 0) * 3600
        + Decimal(parts["minutes"] or 0) * 60
        + Decimal(parts["seconds"] or 0)
    )
...
def parse_time(text):
    """Seconds since midnight of a HH:MM:SS (or HH:MM) time"""
...
def resolve_vehicle_journey(doc, journey, _index=None, _memo=None):
    ...
    for child in reversed(chain[:-1]):
        ...
        if not child.timing_links:
            inherited["timing_links"] = resolved.timing_links
...
def journey_calendar(doc, calendars, service, profile, start, end):
    """
    ...
    the encoded exceptions, or None when the journey never operates.
    """
    ...
    if profile is not None:
        ...
        # union of the organisations' dates
...
def _journey_fingerprint(
    journey, exceptions, period=None, service_period=None, synthetic_route=None
):
    """What distinguishes same-time journeys of one pattern: exceptions, a
    calendar clipped away from the service's period, a line-derived route,
    timing link overrides and frequency data; '' when none of them apply"""
    parts = []
    if exceptions:
        parts.append(exceptions)
    if period is not None and period != service_period:
...
def process_vehicle_journeys(doc, service_jp_info):
    ...
    for i, journey in enumerate(vjourneys):
        ...
        departure_seconds = parse_time(departure_time)
        ...
        if len(sections) == 1:
            ...
            # timing overrides or frequency data must not share a trip
        ...
        if journey.frequency is not None and journey.frequency.end_time:
            ...
            frequency_headway = parse_runtime_duration(journey.frequency.interval)
            if frequency_headway <= 0:
                raise ValueError(
                    "VehicleJourney '%s' has no Frequency interval."
                    % vehicle_journey_id
                )
        else:
        ...
        stop_ids = [timings[0][0].from_stop] + [t[0].to_stop for t in timings]
        route_link_refs = [t[0].route_link_ref for t in timings]
        route_link_refs = route_link_refs[:1] + route_link_refs
