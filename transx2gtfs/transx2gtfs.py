...
def get_stop_info(data):
    ...
    for section in route_sections:
        ...
        for link in route_links:
            ...
            stop_info = stop_info.append(info, ignore_index=True)
...
def get_stops(data, naptan_stops_fp=None):
    ...
    osgb36 = pyproj.Proj("+proj=tmerc +lat_0=49 +lon_0=-2 +k=0.999601 +x_0=400000 +y_0=-100000 +ellps=airy +towgs84=446.448,-125.157,542.060,0.1502,0.2470,0.8421,-20.4894 +units=m +no_defs <>")
    ...
    for p in data.TransXChange.StopPoints.StopPoint:
        ...
        # Add to container
        stop_data = stop_data.append(stop, ignore_index=True)
...
def get_route_links(data):
    ...
    for rs in data.TransXChange.RouteSections.RouteSection:
        ...
        for rl in rlinks:
            ...
            route_links = route_links.append(rlink, ignore_index=True)
    return route_links
...
def get_agency(data):
    ...
    agency_data = agency_data.append(agency, ignore_index=True)
    return agency_data
...
def process(vjourneys):
    ...
    for i, journey in enumerate(vjourneys):
        ...
        gtfs_info = gtfs_info.append(info, ignore_index=True) 
    ...
    for i, journey in enumerate(vjourneys):
        ...
        for section in sections:
            ...
            for link in timing_links:
                ...
                # Convert to string
                arrival_t = "{arrival_hour}:{minsecs}".format(arrival_hour=arrival_hour, minsecs=current_dt.strftime("%M:%S"))
                departure_t = "{departure_hour}:{minsecs}".format(departure_hour=departure_hour, minsecs=departure_dt.strftime("%M:%S"))
                ...
                info = dict(stop_id=stop_id,
                            ...
                            weekdays=weekdays,
                            non_operative_days=non_operative_days)
                section_times = section_times.append(info, ignore_index=True)
                
                # Update stop number
                stop_num += 1
      
            # Add to journey DataFrame
            journey_info = journey_info.append(section_times, ignore_index=True)
            
        # Merge into stop times
        gtfs_info = gtfs_info.append(journey_info, ignore_index=True) 
...
def get_service_journey_pattern_info(data):
    ...
    for service in services:
        ...
        for jp in journey_patterns:
            ...
            # Add to DataFrame
            service_jp_info = service_jp_info.append(row, ignore_index=True)
...
def parse_day_range(dayinfo):
    ...
    # Generate DF
    day_info = day_info.append(row, ignore_index=True)
    return day_info
...
def get_calendar(gtfs_info):
    ...
    for idx, row in calendar.iterrows():
        ...
        # Add to container
        gtfs_calendar = gtfs_calendar.append(dayrow, ignore_index=True)
...
def get_calendar_dates(gtfs_info, bank_holidays_region='england-and-wales'):
    ...
    for idx, row in calendar_info.iterrows():
        ...
        for date in bank_holidays:
            ...
            # Add to container
            calendar_dates = calendar_dates.append(row, ignore_index=True)
...
def main():
    ...
    # Data directory
    data_dir = r"C:\HY-DATA\hentenka\Data\TFL\LULDLRTRAMRIVERCABLE 26022019"
    output_fp = r"C:\HY-DATA\hentenka\Data\TFL\London_metro_GTFS.zip"
    gtfs_db = "data/gtfs.db"
    ...
    # NAPTAN stops
    naptan_stops_fp = r"C:\HY-DATA\hentenka\KOODIT\Uni\transxchange_gtfspy\data\Stops.txt"
    ...
    # Export to disk
    save_to_gtfs_zip(output_zip_fp=output_fp, gtfs_data=gtfs_data)
